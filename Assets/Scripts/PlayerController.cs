...
{
    private LimbsManager limbsManager;
    ...
    void Awake()
    {
        ...
        if (!limbsManager)
            Debug.Log($"{typeof(LimbsManager).Name} not found at {gameObject.name}!");
    ...
    void Update()
    {
        if (inputHandler.Jumped && IsGrounded())
            yVelocity = limbsManager.JumpForce;
    ...
    private void Move()
    {
        ...
        XZMotion *= Mathf.Max(limbsManager.MoveSpeed, minMoveSpeed) * Time.deltaTime;
